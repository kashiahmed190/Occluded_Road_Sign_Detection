...
#   Loss   : 3-region weighted MSE (using the REAL XML-derived
#            bbox_mask -- hidden/visible/background), no SSIM/L1
#   Purpose: ablation -- proves region weighting matters, isolated
#            from Model D's added SSIM/L1/edge terms
# =========================================================
...
BEST_MODEL_PATH   = os.path.join(SAVE_DIR, "best_roi_unet.pth")
FINAL_MODEL_PATH  = os.path.join(SAVE_DIR, "final_roi_unet.pth")
...
def roi_weighted_mse(y_true, y_pred, occ_mask, bbox_mask):
    """Model C -- 3-region weighted MSE (no SSIM/L1 -- isolates the
    contribution of region weighting alone, vs Model B's flat MSE)."""
...
def save_loss_curve(history, path):
    ...
    plt.title('Model C — ROI U-Net (5ch input, 3-region weighted MSE)')
...
def train():
    ...
    model     = ROIUNet(in_ch=5, out_ch=3).to(DEVICE)
    ...
    save_json(CONFIG_JSON_PATH, {
        'model':        'Model C — ROI U-Net',
        'in_channels':  5,
        'loss':         '3-region weighted MSE (real bbox_mask) -- no SSIM, no L1',
    ...
    for epoch in range(EPOCHS):
        ...
        for x, y, occ_mask, bbox_mask, _ in train_loader:
            ...
            pred = model(x)
            loss = roi_weighted_mse(y, pred, occ_mask, bbox_mask)
