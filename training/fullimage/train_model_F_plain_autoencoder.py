...
#   Arch   : encoder-decoder bottleneck, NO skip connections at all
#   Loss   : plain MSE
#   Purpose: isolates exactly how much U-Net-style skip connections
#            are worth, vs Models B-E which all have them
# =========================================================
...
BEST_MODEL_PATH   = os.path.join(SAVE_DIR, "best_model_F.pth")
FINAL_MODEL_PATH  = os.path.join(SAVE_DIR, "final_model_F.pth")
...
def mse_loss_only(y_true, y_pred, occ_mask=None, bbox_mask=None):
    """Model F -- plain MSE, no region weighting (deliberately the
    simplest baseline, isolating how much skip connections are worth)."""
...
class PlainAutoencoder(nn.Module):
    """
    Model F -- Plain Autoencoder.
    Same depth/channel progression as the U-Net family but the decoder
    receives NOTHING from the encoder except the bottleneck. Isolates
    exactly how much U-Net's skip connections are worth.
    """
...
def save_loss_curve(history, path):
    ...
    plt.title('Model F — Plain Autoencoder (no skip connections, plain MSE)')
...
def train():
    ...
    model     = PlainAutoencoder(in_ch=5, out_ch=3).to(DEVICE)
    ...
    save_json(CONFIG_JSON_PATH, {
        'model':        'Model F — Plain Autoencoder',
        'in_channels':  5,
        'loss':         'plain MSE, no region weighting',
    ...
    for epoch in range(EPOCHS):
        ...
        if val_loss < best_val:
            ...
            torch.save({
                ...
                'in_ch':                5,
                'arch':                 'plain_ae',
                'model_name':           'Model F — Plain Autoencoder',
            }, BEST_MODEL_PATH)
