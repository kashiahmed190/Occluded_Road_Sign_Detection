...
# =========================================================
# MODEL B — Mask U-Net
#   Input  : 5 channels (RGB + occ_mask + bbox_mask)   [same as Enhanced]
#   Loss   : plain MSE only                             [dumb loss — no SSIM, no L1, no region weighting]
#   Purpose: ablation — proves the 3-region loss matters
# =========================================================
...
BEST_MODEL_PATH   = os.path.join(SAVE_DIR, "best_mask_unet.pth")
FINAL_MODEL_PATH  = os.path.join(SAVE_DIR, "final_mask_unet.pth")
...
def collate_fn(batch):
    xs, ys, fns = zip(*batch)
    return torch.stack(xs), torch.stack(ys), list(fns)


# =========================================================
# LOSS — Model B: plain MSE only (no region weighting,
#         no SSIM, no L1)
# KEY DIFFERENCE vs Enhanced U-Net
# =========================================================

def mse_loss_only(y_true, y_pred):
    """
    Flat MSE across all pixels — no region awareness.
    The model sees WHERE the mask is (5ch input) but the
    loss treats every pixel equally. This isolates the
    contribution of the 3-region weighted loss.
    """
    return F.mse_loss(y_pred.float(), y_true.float())


# =========================================================
# MODEL — same architecture as Enhanced U-Net
#         (5ch input, identical encoder-decoder)
# =========================================================
...
class MaskUNet(nn.Module):
    """
    Identical architecture to EnhancedUNet.
    Only difference: trained with plain MSE loss.
    """
...
def save_loss_curve(history, path):
    ...
    plt.title('Model B — Mask U-Net (5ch input, MSE loss only)')
...
def train():
    ...
    model     = MaskUNet(in_ch=5, out_ch=3).to(DEVICE)
    ...
    save_json(CONFIG_JSON_PATH, {
        'model':        'Model B — Mask U-Net',
        'in_channels':  5,
        'loss':         'MSE only (no region weighting, no SSIM, no L1)',
        ...
        'ablation_note': '5ch input same as Enhanced; loss is flat MSE to isolate loss contribution',
    })
    ...
    for epoch in range(EPOCHS):
        ...
        for x, y, _ in train_loader:
            ...
            pred = model(x)
            loss = mse_loss_only(y, pred)   # <-- plain MSE only
