...
#   Loss   : 3-region weighted MSE (real bbox_mask) + SSIM + L1
#            on the hidden region
#   Purpose: ablation -- proves structural/perceptual loss terms
#            add value on top of Model C's region weighting alone
# =========================================================
...
SAVE_DIR        = "enhanced_unetapril2026"
...
# Region loss weights (used by loss functions that region-weight)
W_HIDDEN  = 1.0
W_VISIBLE = 0.4
W_BG      = 0.1

BEST_MODEL_PATH   = os.path.join(SAVE_DIR, "best_roi_unet_ssim.pth")
FINAL_MODEL_PATH  = os.path.join(SAVE_DIR, "final_roi_unet_ssim.pth")
...
class EnhancedUNetDataset(Dataset):
...
def region_weights(occ_mask, bbox_mask):
    """Uses the REAL XML-derived bbox_mask for 3-region weighting."""
    occ  = occ_mask.float()
...
def enhanced_loss(y_true, y_pred, occ_mask, bbox_mask):
    """Model D -- 3-region MSE + SSIM + L1 on the hidden region.
    KEY DIFFERENCE vs Model C: adds structural (SSIM) and sharp-edge
    (L1) terms on top of the same region weighting."""
    w    = region_weights(occ_mask, bbox_mask).expand_as(y_pred)
    mse  = (w * (y_pred - y_true) ** 2).mean()
    ss   = ssim_loss(y_pred, y_true)
    hidden = (occ_mask * bbox_mask).expand_as(y_pred)
    n    = hidden.sum().clamp(min=1)
    l1   = ((y_pred - y_true).abs() * hidden).sum() / n
...
def save_loss_curve(history, path):
    ...
    plt.title('Model D — Enhanced U-Net (5ch input, region MSE + SSIM + L1)')
...
def train():
    ...
    model     = EnhancedUNet(in_ch=5, out_ch=3).to(DEVICE)
    ...
    save_json(CONFIG_JSON_PATH, {
        'model':        'Model D — Enhanced U-Net',
        'in_channels':  5,
        'loss':         'region-weighted MSE (real bbox_mask) + SSIM + L1 (hidden region)',
    ...
    for epoch in range(EPOCHS):
        ...
        for x, y, occ_mask, bbox_mask, _ in train_loader:
            ...
            pred = model(x)
            loss = enhanced_loss(y, pred, occ_mask, bbox_mask)
        ...
        if val_loss < best_val:
            ...
            torch.save({
                ...
                'in_ch':                5,
                'arch':                 'unet',
                'model_name':           'Model D — Enhanced U-Net',
            }, BEST_MODEL_PATH)
